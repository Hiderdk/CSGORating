...
dfa = pd.read_pickle(r"C:\Users\Mathias\PycharmProjects\Ratings\Files\all_game_all_player_performance_rating")

df = dfa[dfa['player_name']=='device']

resampled = df.resample('D', on='start_date_time')['opponent_adjusted_performance_rating'].mean().reset_index()
import torch
...
# Train the model
class da_rnn:
    def __init__(self,dat,  encoder_hidden_size = 64, decoder_hidden_size = 64, T = 10,
...
io_dir = '~/nasdaq'
df = pd.read_csv(r"C:\Users\Mathias\Documents\Downloads\nasdaq100\nasdaq100_padding.csv")
model = da_rnn(df, parallel = False,
              learning_rate = .001)
