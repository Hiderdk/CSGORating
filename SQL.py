...
def get_all_time_game_player_data(min_date,max_date):
    ...
    gt.team_id,map,g.game_number,adr,gt.won,gt.kills as team_kills,gp.kills/gt.kills as kill_percentage,gt.team_id_opponent,s.format
    from game_player gp 
...
def  get_all_game_all_player_from_series_ids(series_ids):
    ...
    gt.kills as team_kills,gp.kills/gt.kills as kill_percentage,team_id_opponent,s.format
    from game_player gp 
    join game g on g.id=gp.game_id
    join game_team gt on gt.game_id = gp.game_id and gp.team_id = gt.team_id
    join player p on p.id = gp.player_id 
